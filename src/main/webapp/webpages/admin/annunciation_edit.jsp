    ...
    <div class="text-left col-sm-10 panel panel-primary">
        ...
        <form class="form-horizontal" role="form" action="">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="name" id="title" value="" placeholder="请输入公告标题">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="remark" class="col-sm-2 control-label">公告备注</label>
                <div class="col-sm-10">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var annunciationId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var annunciations = getAnnunciationById(annunciationId);
        /*初始化编辑数据*/
        editDataInitialization(annunciations);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(annunciations) {
        if (annunciations === null || annunciations === '') {
        ...
        $("#id").val(annunciations.id);
        $("#title").val(annunciations.title);
        $("#content").html(annunciations.content);
        $("#remarks").val(annunciations.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getAnnunciationById(id) {
        ...
        var annunciation = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/annunciation/getAnnunciationById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    annunciation = jsonData.data.annunciation;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">公告不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
        ...
        $.ajax({
            url: "${webRoot}/annunciation/update",
            type: "post",
            data: data,
            dataType: "json",
            success: function (data) {
