...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var spotTeachingId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var spotTeaching = getSpotTeachingById(spotTeachingId);
        /*初始化编辑数据*/
        editDataInitialization(spotTeaching);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(spotTeaching) {
        if (spotTeaching === null || spotTeaching === '') {
        ...
        $("#id").val(spotTeaching.id);
        $("#title").val(spotTeaching.title);
        $("#content").html(spotTeaching.content);
        $("#imgs").val(spotTeaching.imgs);
        $("#remarks").val(spotTeaching.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getSpotTeachingById(id) {
        ...
        var spotTeaching = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/spotTeaching/getSpotTeachingById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    spotTeaching = jsonData.data.spotTeaching;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">现场讲解不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
