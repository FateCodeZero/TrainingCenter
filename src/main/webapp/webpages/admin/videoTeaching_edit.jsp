...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var videoTeachingId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var videoTeaching = getVideoTeachingById(videoTeachingId);
        /*初始化编辑数据*/
        editDataInitialization(videoTeaching);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(videoTeaching) {
        if (videoTeaching === null || videoTeaching === '') {
        ...
        $("#id").val(videoTeaching.id);
        $("#title").val(videoTeaching.title);
        $("#content").html(videoTeaching.content);
        $("#imgs").val(videoTeaching.imgs);
        $("#remarks").val(videoTeaching.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getVideoTeachingById(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (code === 1) {
                    videoTeaching = jsonData.data.videoTeaching;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">影音教学不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
