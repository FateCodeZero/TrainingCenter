...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var interviewTeachingId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var interviewTeaching = getInterviewTeachingById(interviewTeachingId);
        /*初始化编辑数据*/
        editDataInitialization(interviewTeaching);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(interviewTeaching) {
        if (interviewTeaching === null || interviewTeaching === '') {
        ...
        $("#id").val(interviewTeaching.id);
        $("#title").val(interviewTeaching.title);
        $("#content").html(interviewTeaching.content);
        $("#imgs").val(interviewTeaching.imgs);
        $("#remarks").val(interviewTeaching.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getInterviewTeachingById(id) {
        ...
        var interviewTeaching = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/interviewTeaching/getInterviewTeachingById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    interviewTeaching = jsonData.data.interviewTeaching;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">访谈 教学不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
        ...
        $.ajax({
            url: "${webRoot}/interviewTeaching/update",
            type: "post",
            data: data,
            dataType: "json",
            success: function (data) {
