...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var revolutionarySiteId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var revolutionarySite = getRevolutionarySiteById(revolutionarySiteId);
        /*初始化编辑数据*/
        editDataInitialization(revolutionarySite);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(revolutionarySite) {
        if (revolutionarySite === null || revolutionarySite === '') {
        ...
        $("#id").val(revolutionarySite.id);
        $("#title").val(revolutionarySite.title);
        $("#content").html(revolutionarySite.content);
        $("#imgs").val(revolutionarySite.imgs);
        $("#remarks").val(revolutionarySite.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getRevolutionarySiteById(id) {
        ...
        var revolutionarySite = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/revolutionarySite/getRevolutionarySiteById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    revolutionarySite = jsonData.data.revolutionarySite;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">内容不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
        ...
        $.ajax({
            url: "${webRoot}/revolutionarySite/update",
            type: "post",
            data: data,
            dataType: "json",
            success: function (data) {
