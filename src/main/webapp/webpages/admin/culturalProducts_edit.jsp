...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var culturalProductsId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var culturalProducts = getCulturalProductsById(culturalProductsId);
        /*初始化编辑数据*/
        editDataInitialization(culturalProducts);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(culturalProducts) {
        if (culturalProducts === null || culturalProducts === '') {
        ...
        $("#id").val(culturalProducts.id);
        $("#title").val(culturalProducts.title);
        $("#content").html(culturalProducts.content);
        $("#imgs").val(culturalProducts.imgs);
        $("#remarks").val(culturalProducts.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getCulturalProductsById(id) {
        ...
        var culturalProducts = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/culturalProducts/getCulturalProductsById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    culturalProducts = jsonData.data.culturalProducts;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        $.ajax({
            url: "${webRoot}/culturalProducts/update",
            type: "get",
            data: data,
            dataType: "json",
            success: function (data) {
