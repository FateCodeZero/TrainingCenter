...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var specialFoodId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var specialFood = getSpecialFoodById(specialFoodId);
        /*初始化编辑数据*/
        editDataInitialization(specialFood);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(specialFood) {
        if (specialFood === null || specialFood === '') {
        ...
        $("#id").val(specialFood.id);
        $("#title").val(specialFood.title);
        $("#content").html(specialFood.content);
        $("#imgs").val(specialFood.imgs);
        $("#remarks").val(specialFood.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getSpecialFoodById(id) {
        ...
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/specialFood/getSpecialFoodById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    specialFood = jsonData.data.specialFood;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        $.ajax({
            url: "${webRoot}/specialFood/update",
            type: "post",
            data: data,
            dataType: "json",
            success: function (data) {
