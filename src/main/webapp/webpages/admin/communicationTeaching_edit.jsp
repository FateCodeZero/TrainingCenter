...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var communicationTeachingId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var communicationTeaching = getCommunicationTeachingById(communicationTeachingId);
        /*初始化编辑数据*/
        editDataInitialization(communicationTeaching);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(communicationTeaching) {
        if (communicationTeaching === null || communicationTeaching === '') {
        ...
        $("#id").val(communicationTeaching.id);
        $("#title").val(communicationTeaching.title);
        $("#content").html(communicationTeaching.content);
        $("#imgs").val(communicationTeaching.imgs);
        $("#remarks").val(communicationTeaching.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getCommunicationTeachingById(id) {
        ...
        var communicationTeaching = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/communicationTeaching/getCommunicationTeachingById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    communicationTeaching = jsonData.data.communicationTeaching;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">与创业名人/团队交流调研教学不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
        ...
        $.ajax({
            url: "${webRoot}/communicationTeaching/update",
            type: "get",
            data: data,
            dataType: "json",
            success: function (data) {
