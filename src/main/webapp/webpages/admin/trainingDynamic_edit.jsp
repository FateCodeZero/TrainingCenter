    ...
    <div class="text-left col-sm-10 panel panel-primary">
        ...
        <form class="form-horizontal" role="form" action="">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="name" id="title" value="" placeholder="请输入培训标题">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="imgs" class="col-sm-2 control-label">培训图片</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="imgs" id="imgs" value="" placeholder="请给出图片链接">
                </div>
            </div>
            <div class="form-group">
                <label for="remark" class="col-sm-2 control-label">培训动态备注</label>
                <div class="col-sm-10">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        wangEditorGet();
        /*从URL获取对象ID*/
        var trainingDynamicId = getUrlParam('id');
        /*通过ID获取对象信息*/
        var trainingDynamic = getTrainingDynamicIdById(trainingDynamicId);
        /*初始化编辑数据*/
        editDataInitialization(trainingDynamic);
    });
     ...
     * @param role：要编辑的角色对象
     */
    function editDataInitialization(trainingDynamic) {
        if (trainingDynamic === null || trainingDynamic === '') {
        ...
        $("#id").val(trainingDynamic.id);
        $("#title").val(trainingDynamic.title);
        $("#content").html(trainingDynamic.content);
        $("#imgs").val(trainingDynamic.imgs);
        $("#remarks").val(trainingDynamic.remarks);
    }
     ...
     * @param id 新闻id
     */
    function getTrainingDynamicIdById(id) {
        ...
        var trainingDynamic = null;
        var data = {id: id};
        $.ajax({
            url: "${webRoot}/trainingDynamic/getTrainingDynamicById",
            type: "get",
            async: false,    //关闭异步请求
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (code === 1) {
                    trainingDynamic = jsonData.data.trainingDynamic;
                } else {
                    layer.alert(msg, {
                        time: 3000,
                        icon: 2
    ...
    $("#submit").click(function () {
        ...
        if (id === null || id === ''){
            var msg = '<div style="text-align: center"><span style="color: #FF5722;font-size: large">培训动态不存在或已被删除，</span><br>即将返回列表界面。</div>';
            layer.confirm(msg, {
                    btn: ['确定', '取消']//按钮
                }
                , function () { //确定之后执行
        ...
        $.ajax({
            url: "${webRoot}/trainingDynamic/update",
            type: "post",
            data: data,
            dataType: "json",
            success: function (data) {
